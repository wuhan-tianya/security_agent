...
### 2026-03-04
- 模型选择统一以后端配置为准：忽略前端传入的 `model` 参数，固定使用 `settings.json` / 环境变量中的 `llm_model`。
- 历史上下文读取改为可配置窗口：`memory_context_message_limit`（默认 30 条消息），避免仅携带单轮对话。
- 当前输入优先级提升：渲染提示词时将“当前用户请求”追加到末尾作为最高优先级。
- 历史上下文清洗：移除历史中的 `<function_calls>` 片段，避免旧工具调用指令污染新一轮决策。
- 流式链路改为真实 token 透传：工具调用闭环后调用模型 `stream=true`，后端逐 token 通过 SSE 推送前端。
- 增加前端 `run_finished` 兜底同步：若流式中断或丢 token，使用后端 `final_response` 覆盖/补齐助手消息。
- 修复前端构建阻塞项：移除未使用的 `ChevronRight` 导入，保证 `build` 可通过。
- 流式解析增加防重复与熔断：兼容累计式 chunk，过滤重复尾段并在无进展重复时主动中断，避免“最后一段死循环发送”。
- 增加“进行中回复”重试状态机：若模型仅返回进度文本（如“分析中/请稍候”），后端会继续与模型交互并推动到最终结论/报告链接后再结束任务。
- 持久化一致性修复：将实际已发送给前端的 `llm_token` 重新拼接后写入数据库，确保刷新后与前端实时显示一致。
